...
std::tuple<double, double, size_t> run_HGMS_hIgG_separation(realtype kf_ion,
    ...
    // Generally, everything happens in SI units, i.e., mol/m³ for concentrations, m³ for volumes, m² for areas, etc.
    // The only exceptions are the Truesdell-Jones activity model parameters, which are in nm and L/mol and
    // that the values in the y vector which are in kg are converted to g/L and mol/L for the reactions
    ...
    ComponentSystem cs{
        {// Format: Component(name).setCharge(...).setMolarMass(...).setTruesdellJonesAlpha(...).setTruesdellJonesBeta(...)
         ...
         //  Component("Mg²⁺").setCharge(2).setTruesdellJonesParameters(5.5e-10, 0.2e-3),  // Only for testing of Truessdell-Jones model -> plays no role in process
         ...
         Component("GlyH₂⁺").setCharge(1).setMolarMass(
             75.067e-3),  // TODO: No truesdell_jones paramters available -> use Debye-Hückel model and see if it makes a difference
         ...
         Component("MNP-OH₂⁺")
             ...
             .setMolarMass(
                 18.015e-3),  // MNP hydroxyl groups are not noted as charged because they should not be considered in the ionic strength of the activity model and in gouy chapman equation
         Component("MNP-OH").setType(Type::MagneticNanoParticleGroup).setMolarMass(17.007e-3),
         Component("MNP-O⁻")
             .setType(Type::MagneticNanoParticleGroup)
             .setMolarMass(
                 15.999e-3),  // MNP hydroxyl groups are not noted as charged because they should not be considered in the ionic strength of the activity model and in gouy chapman equation
    ...
    realtype c_W = 1000 / cs("H₂O").molarMass;  // [mol/m³] - Concentration of water
    ...
    // reactionSystem.add(massActionLaw(cs, "H₂O <=> H⁺ + OH⁻", kf_ion, kb_W, cs.getIdx("H⁺")));
    // reactionSystem.add(massActionLaw(cs, "TrisH⁺ <=> Tris + H⁺", kf_ion, kb_Tris, cs.getIdx("H⁺")));
    // reactionSystem.add(massActionLaw(cs, "AcH <=> Ac⁻ + H⁺", kf_ion, kb_AcH, cs.getIdx("H⁺")));
    // reactionSystem.add(massActionLaw(cs, "GlyH₂⁺ <=> GlyH + H⁺", kf_ion, kb_GlyH2, cs.getIdx("H⁺")));
    // reactionSystem.add(massActionLaw(cs, "GlyH <=> Gly⁻ + H⁺", kf_ion, kb_GlyH, cs.getIdx("H⁺")));

    // reactionSystem.add(massActionLaw_damped(cs, "H₂O <=> H⁺ + OH⁻", kf_ion, kb_W, tau_reaction, cs.getIdx("H⁺")));
    // reactionSystem.add(massActionLaw_damped(cs, "TrisH⁺ <=> Tris + H⁺", kf_ion, kb_Tris, tau_reaction, cs.getIdx("H⁺")));
    // reactionSystem.add(massActionLaw_damped(cs, "AcH <=> Ac⁻ + H⁺", kf_ion, kb_AcH, tau_reaction, cs.getIdx("H⁺")));
    // reactionSystem.add(massActionLaw_damped(cs, "GlyH₂⁺ <=> GlyH + H⁺", kf_ion, kb_GlyH2, tau_reaction, cs.getIdx("H⁺")));
    // reactionSystem.add(massActionLaw_damped(cs, "GlyH <=> Gly⁻ + H⁺", kf_ion, kb_GlyH, tau_reaction, cs.getIdx("H⁺")));

    reactionSystem.add(massActionLaw_inverseRatePrediction(cs, "H₂O <=> H⁺ + OH⁻", Ks_W, tau_reaction, cs.getIdx("H⁺")));
    reactionSystem.add(
        massActionLaw_inverseRatePrediction(cs, "TrisH⁺ <=> Tris + H⁺", Ks_TrisH_plus, tau_reaction, cs.getIdx("H⁺")));
    reactionSystem.add(massActionLaw_inverseRatePrediction(cs, "AcH <=> Ac⁻ + H⁺", Ks_AcH, tau_reaction, cs.getIdx("H⁺")));
    reactionSystem.add(
        massActionLaw_inverseRatePrediction(cs, "GlyH₂⁺ <=> GlyH + H⁺", Ks_GlyH2_plus, tau_reaction, cs.getIdx("H⁺")));
    reactionSystem.add(
        massActionLaw_inverseRatePrediction(cs, "GlyH <=> Gly⁻ + H⁺", Ks_GlyH, tau_reaction, cs.getIdx("H⁺")));

    // TODO: maybe try smaller time constant for gouy chapman reaction
    // auto reaction_OH2_plus = massActionLaw(cs, "MNP-OH₂⁺ <=> MNP-OH + H⁺", kf_ion, kb_MNP_OH2_plus, cs.getIdx("H⁺"));
    // auto reaction_OH = massActionLaw(cs, "MNP-OH <=> MNP-O⁻ + H⁺", kf_ion, kb_MNP_OH, cs.getIdx("H⁺"));

    // auto reaction_OH2_plus = massActionLaw_damped(cs, "MNP-OH₂⁺ <=> MNP-OH + H⁺", kf_ion, kb_MNP_OH2_plus, tau_reaction,
    //                                               cs.getIdx("H⁺"));
    // auto reaction_OH = massActionLaw_damped(cs, "MNP-OH <=> MNP-O⁻ + H⁺", kf_ion, kb_MNP_OH, tau_reaction, cs.getIdx("H⁺"));
    ...
    //             reaction_H_plus_gouy_chapman(t, concentrations, modified_activities_map);
    //             auto error_OH2_plus = reaction_OH2_plus.errorFunction(t, concentrations,
    ...
    reactionSystem.add(Reaction(
        [
            ...
            idx_MNP_hIgG = cs.getIdx("MNP-hIgG"), idx_hIgG = cs.getIdx("hIgG"), M_hIgG = cs("hIgG").molarMass,
            ...
            // Devide by molar mass of Protein to account for q being m_MNP_hIgG / m_MNP -> (q_max - c_MNP_hIgG / c_MNP) * c_MNP / M_hIgG gives molar concentration of binding sites
            // rate = k_ads * c_hIgG * (q_max - q) * (c_MNP / M_hIgG) - k_des * c_MNP_hIgG = 0 at equilibrium
            // k_ads + (c_hIgG - r) * (q_max - (C_MNP_hIgG + r) / c_MNP) * (c_MNP / M_hIgG) - k_des * (c_MNP_hIgG + r) = 0
            ...
            // const auto residual_current = K_B * c_hIgG * (q_max - c_MNP_hIgG / c_MNP) * (c_MNP / M_hIgG) - c_MNP_hIgG;
            ...
            // const auto residual_eq_1 = K_B * c_hIgG_eq_1 * (q_max - c_MNP_hIgG_eq_1 / c_MNP) * (c_MNP / M_hIgG) -
            //                            c_MNP_hIgG_eq_1;
            ...
            // const auto residual_eq_2 = K_B * c_hIgG_eq_2 * (q_max - c_MNP_hIgG_eq_2 / c_MNP) * (c_MNP / M_hIgG) -
            //                            c_MNP_hIgG_eq_2;
            ...
            assert(((c_hIgG_eq_1 - 1e-12) < 0).all() ||
                   ((c_MNP_hIgG_eq_1 - 1e-12) < 0).all() &&
                       "Equilibrium concentrations (solution 1) should contain negative values");

            // Skip these assertions because they are fragile e.g. for extremely dilute solutions, etc.

            // assert((Q_relative_error_2 < 1e-1).all() &&
            //        "Equilibrium concentrations (solution 2) must satisfy equilibrium condition");
            ...
            //        "Equilibrium concentrations (solution 2) must be closer to equilibrium than current concentrations");
        }));
    ...
    if (std::abs(std::accumulate(mnp_distribution, mnp_distribution + 10, 0.0) - 1.0) > 1e-3) {
        throw std::runtime_error("Error: MNP distribution does not sum to 1 but to " +
                                 std::to_string(std::accumulate(mnp_distribution, mnp_distribution + 10, 0.0)) + "!");
    }
    ...
    feed(cs.getIdx(
        "MNP-OH")) = MNP_Ns * MNP_specific_surface *
                     c_MNP_feed;  // MNP_Ns * MNP_specific_surface is in mol/kg, multiply with c_MNP_feed in kg/m³ to get mol/m³

    std::cout << "Feed before reaction: " << feed << std::endl;
    realtype feed_error;
    std::tie(feed, feed_error) = oneCellReaction(reactionSystem, feed, t_reaction_duration * 10, solverType,
                                                 timeout_seconds);
    ...
    std::tie(buffer1, buffer1_error) = oneCellReaction(reactionSystem, buffer1, t_reaction_duration, solverType,
                                                       timeout_seconds);
    ...
    std::tie(buffer2, buffer2_error) = oneCellReaction(reactionSystem, buffer2, t_reaction_duration, solverType,
                                                       timeout_seconds);
    std::cout << "B2 after reaction: " << buffer2 << std::endl;
    auto pH_B2 = -std::log10(buffer2(cs.getIdx("H⁺")) * 1e-3);
    std::cout << "pH B2: " << pH_B2 << " (should be ~4.75 (Goal/Paper) / 4.77 (Matlab))" << std::endl;
    ...
    std::tie(buffer3, buffer3_error) = oneCellReaction(reactionSystem, buffer3, t_reaction_duration, solverType,
                                                       timeout_seconds);
    std::cout << "B3 after reaction: " << buffer3 << std::endl;
    auto pH_B3 = -std::log10(buffer3(cs.getIdx("H⁺")) * 1e-3);
    std::cout << "pH B3: " << pH_B3 << " (should be ~2.5 (Goal/Paper) / 2.41 (Matlab))" << std::endl;
    ...
    std::tie(buffer5_water, buffer5_water_error) = oneCellReaction(reactionSystem, buffer5_water, t_reaction_duration,
    ...
    reactionSystem.max_error = 0.0;  // Set back to default for following main simulation
    ...
    recipe.push_back({"Wash 3 recapture loop", 196, 30, 0, Loop, buffer5_water, nullptr});
    ...
    D_ax_curve << D_ax_pipes, 1.3e-5, 5e-5, 6.9e-5, 8.4e-5;  // TODO: inconsistent with 0.5e-6 for 60% in paper
    ...
    // Capture function: 1.0 for magnetic capture, negative values for uncapture i.e. dm_dt = capture_function(t) * m
    ...
    auto a_eff_function = [](realtype um_u0_ratio) {
        ...
        auto p = 0.07477;
        auto q = 1.083;
        return 1 / (1 + a1 * std::exp(-p * um_u0_ratio) + a2 * std::exp(-q * um_u0_ratio) + a3);
    };
    ...
    auto pc = std::make_shared<RS_MagneticCaptureProcessChamber>(
           ...
           19,  // alpha_fluid_particle_volume_ratio - Slurry factor ratio of fluid volume to MNP volume in the slurry
           2,   // deposition_rate - Empirical parameter in G to account for deposition rate
           MNP_capacity,             // MNP_capacity - Maximum mass of magnetic particles with that can be captured
           magnetic_field_strength,  // T_halfLife_uncapture - In the uncapture case, half of the MNPs in slurry phase decay back to liquid phase every T_halfLife seconds
    ...
    std::size_t n_obs_time_steps = static_cast<std::size_t>(total_duration / dt_obs) + 1;
    ...
    if (save_obs) {
        ...
        obs_frac_feed_1.save_to_npz(obs_unitOperations_filename, "frac_feed_1", "a");
        obs_frac_feed_2.save_to_npz(obs_unitOperations_filename, "frac_feed_2", "a");
        obs_frac_wash_1.save_to_npz(obs_unitOperations_filename, "frac_wash_1", "a");
        obs_frac_wash_2.save_to_npz(obs_unitOperations_filename, "frac_wash_2", "a");
        obs_frac_wash_3.save_to_npz(obs_unitOperations_filename, "frac_wash_3", "a");
        obs_frac_elution_1.save_to_npz(obs_unitOperations_filename, "frac_elution_1", "a");
        obs_frac_elution_2.save_to_npz(obs_unitOperations_filename, "frac_elution_2", "a");
        obs_frac_elution_3.save_to_npz(obs_unitOperations_filename, "frac_elution_3", "a");
        obs_frac_elution_4.save_to_npz(obs_unitOperations_filename, "frac_elution_4", "a");
        obs_frac_elution_5.save_to_npz(obs_unitOperations_filename, "frac_elution_5", "a");
    ...
    return std::make_tuple(reactionSystem.max_error, t_solve_duration, internal_time_stamps.size());
}
