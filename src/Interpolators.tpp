...
template <typename VectorType> class LinearInterpolator {
public:
  LinearInterpolator(const VectorType &x, const VectorType &y) : x(x), y(y) {
    if (x.size() != y.size() || x.size() < 2) {
      throw std::invalid_argument(
          "Vectors must have the same size and at least two elements.");
    }
  }

  typename VectorType::Scalar
  operator()(typename VectorType::Scalar x_query) const {
