...
def build_component_system(denaturation: bool = False) -> fs3.ComponentSystem:
    """``denaturation=True`` adds the denatured-hIgG component (see reactions.build_reaction_system)."""
    components = [
    ...
    if denaturation:
        # Must stay inside the non-magnetic block, which FS³ takes as everything up to hIgG.
        components.insert(next(i for i, c in enumerate(components) if c.name == "hIgG"),
                          fs3.Component("hIgG-den", molar_mass_kg_per_mol=150.0))
    return fs3.ComponentSystem(components)
