...
std::tuple<double, double, size_t> run_HGMS_hIgG_separation(realtype kf_ion,
    ...
    ReactionSystem rs(cs, acitivityModel);
    ...
    // // Classical mass action law reactions
    // rs.add(massActionLaw(cs, "H₂O <=> H⁺ + OH⁻", kf_ion, kf_ion / Ks_W, cs.getIdx("H⁺")));
    // rs.add(massActionLaw(cs, "TrisH⁺ <=> Tris + H⁺", kf_ion, kf_ion / Ks_TrisH_plus, cs.getIdx("H⁺")));
    // rs.add(massActionLaw(cs, "AcH <=> Ac⁻ + H⁺", kf_ion, kf_ion / Ks_AcH, cs.getIdx("H⁺")));
    // rs.add(massActionLaw(cs, "GlyH₂⁺ <=> GlyH + H⁺", kf_ion, kf_ion / Ks_GlyH2_plus, cs.getIdx("H⁺")));
    // rs.add(massActionLaw(cs, "GlyH <=> Gly⁻ + H⁺", kf_ion, kf_ion / Ks_GlyH, cs.getIdx("H⁺")));

    // // Damped mass action law reactions
    // rs.add(massActionLaw_damped(cs, "H₂O <=> H⁺ + OH⁻", kf_ion, kf_ion / Ks_W, tau_reaction, cs.getIdx("H⁺")));
    // rs.add(massActionLaw_damped(cs, "TrisH⁺ <=> Tris + H⁺", kf_ion, kf_ion / Ks_TrisH_plus, tau_reaction, cs.getIdx("H⁺")));
    // rs.add(massActionLaw_damped(cs, "AcH <=> Ac⁻ + H⁺", kf_ion, kf_ion / Ks_AcH, tau_reaction, cs.getIdx("H⁺")));
    // rs.add(massActionLaw_damped(cs, "GlyH₂⁺ <=> GlyH + H⁺", kf_ion, kf_ion / Ks_GlyH2_plus, tau_reaction, cs.getIdx("H⁺")));
    // rs.add(massActionLaw_damped(cs, "GlyH <=> Gly⁻ + H⁺", kf_ion, kf_ion / Ks_GlyH, tau_reaction, cs.getIdx("H⁺")));

    // Inverse rate prediction mass action law reactions
    rs.add(massActionLaw_inverseRatePrediction(cs, "H₂O <=> H⁺ + OH⁻", Ks_W, tau_reaction, cs.getIdx("H⁺")));
    rs.add(massActionLaw_inverseRatePrediction(cs, "TrisH⁺ <=> Tris + H⁺", Ks_TrisH_plus, tau_reaction, cs.getIdx("H⁺")));
    rs.add(massActionLaw_inverseRatePrediction(cs, "AcH <=> Ac⁻ + H⁺", Ks_AcH, tau_reaction, cs.getIdx("H⁺")));
    rs.add(massActionLaw_inverseRatePrediction(cs, "GlyH₂⁺ <=> GlyH + H⁺", Ks_GlyH2_plus, tau_reaction, cs.getIdx("H⁺")));
    rs.add(massActionLaw_inverseRatePrediction(cs, "GlyH <=> Gly⁻ + H⁺", Ks_GlyH, tau_reaction, cs.getIdx("H⁺")));

    // 2pK Reactions for MNP surface groups - using Gouy-Chapman model for surface concentration of H⁺

    // auto reaction_OH2_plus = massActionLaw(cs, "MNP-OH₂⁺ <=> MNP-OH + H⁺", kf_ion, kf_ion / Ks_MNP_OH2_plus, cs.getIdx("H⁺"));
    // auto reaction_OH = massActionLaw(cs, "MNP-OH <=> MNP-O⁻ + H⁺", kf_ion, kf_ion / Ks_MNP_OH, cs.getIdx("H⁺"));
    ...
    realtype feed_error;
    std::tie(feed, feed_error) = oneCellReaction(rs, feed, t_reaction_duration * 10, solverType, timeout_seconds);
    ...
    std::tie(buffer1, buffer1_error) = oneCellReaction(rs, buffer1, t_reaction_duration, solverType, timeout_seconds);
    ...
    std::tie(buffer2, buffer2_error) = oneCellReaction(rs, buffer2, t_reaction_duration, solverType, timeout_seconds);
    ...
    std::tie(buffer3, buffer3_error) = oneCellReaction(rs, buffer3, t_reaction_duration, solverType, timeout_seconds);
    ...
    rs.max_error = 0.0;  // Set back to default for following main simulation

    // =============================================================
    // ========================== RECIPE ===========================
    // =============================================================

    auto frac_feed_1 = std::make_shared<Volume>(rs);
    auto frac_feed_2 = std::make_shared<Volume>(rs);
    auto frac_wash_1 = std::make_shared<Volume>(rs);
    auto frac_wash_2 = std::make_shared<Volume>(rs);
    auto frac_wash_3 = std::make_shared<Volume>(rs);
    auto frac_elution_1 = std::make_shared<Volume>(rs);
    auto frac_elution_2 = std::make_shared<Volume>(rs);
    auto frac_elution_3 = std::make_shared<Volume>(rs);
    auto frac_elution_4 = std::make_shared<Volume>(rs);
    auto frac_elution_5 = std::make_shared<Volume>(rs);
    ...
    auto inlet = std::make_shared<Inlet>(rs, inlet_function);
    ...
    auto dead_volume = std::make_shared<Volume>(rs, V_dead);
    ...
    process.addConnection(pipe_loop->exit(), pipe_inlet->entry(), flowRate_function_loop);
    ...
    std::cout << "Max pH error: " << rs.max_error << std::endl;
