"""Shared FS³ model construction (component system, reactions, solutions).

These builders are used both by the standalone simulation script (``HGMS_hIgG_separation.py``) and by the UI / plot scripts, 
so that the component system, reactions and buffer solutions are defined in exactly one place.
"""
...
import fs3
from shared_data import Solution
...
def build_solutions(cs, rs, solver_type, timeout_seconds=float("inf")):
    """Build and equilibrate the inlet solutions; returns ``{name: Solution}``."""
    c_w = 1000.0 / cs["H₂O"].molar_mass
    ...
    solutions = [
        Solution("Feed", feed),
        Solution("Buffer 1", buffer1),
        Solution("Buffer 2", buffer2),
        Solution("Buffer 3", buffer3),
        Solution("Water (B5)", water),
    ]
    return {sol.name: sol for sol in solutions}
